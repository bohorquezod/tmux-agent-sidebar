...
_exec_cmd() {
  ...
  case "$_cmd" in
    ...
    :move)
      ...
      if [[ "$_src" =~ ^([0-9]+)\.([0-9]+)$ ]]; then
        ...
        if [[ $_delta -gt 0 ]]; then
          local _d=0
          while [[ $_d -lt $_delta ]]; do
            move_window_down $_src_idx
            ((_src_idx++))
            ((_d++))
          done
  ...
  [[ "$_cn" == :* ]] && _cn="${_cn#:}"
  if [[ "$_cn" =~ ^([0-9]+)\.([0-9]+)$ ]]; then
    _snum="${BASH_REMATCH[1]}"
    _wnum="${BASH_REMATCH[2]}"
  elif [[ "$_cn" =~ ^[0-9]+$ ]]; then
    _snum="$_cn"
  else
    return
