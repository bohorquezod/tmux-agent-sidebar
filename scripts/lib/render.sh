    ...
    if [[ "$_itype" == "S" ]]; then
      ...
      if [[ "$_srv" != "$prev_server" ]]; then
        if [[ "$_drill_mode" == "0" ]]; then
          ...
          local _sic="$GR"
          [[ "$_is_cur" == "1" ]] && _sic="$CY"
          local _srvd="${_srv:0:$max}"
          [[ ${#_srv} -gt $max ]] && _srvd="${_srv:0:$((max - 1))}…"
      ...
      [[ -n "$_KILL_PENDING" && "$_item" == "$_KILL_PENDING" ]] && {
        _ic="$RD"
        _nc="$RD"
        _cursor="✕"
      }
      local _sessd="${_sess:0:$max}"
      [[ ${#_sess} -gt $max ]] && _sessd="${_sess:0:$((max - 1))}…"
      if [[ "$_drill_mode" == "1" ]]; then
      ...
      case "$_state" in
        ...
        idle)
          ...
          ;;
        working)
          _display_icon="${_SPINNER[$_SPIN_FRAME]}"
          _icon_col="$CY"
          _name_col="$CY"
          ;;
        blocked)
