  ...
  if [[ "$_SEARCH_MODE" == "1" ]]; then
    case "$key" in
      ...
      k | UP)
        ...
        ;;
      *)
        if [[ ${#key} -eq 1 && "$key" > $'\x1f' && "$key" != $'\x7f' ]]; then
          _SEARCH_QUERY+="$key"
          _SEARCH_SEL=0
        fi
        ;;
  ...
  case "$key" in
    ...
    r)
      ...
      if [[ "$_rtype" == "S" ]]; then
        ...
        local _rname
        _rname=$("${_rtmux[@]}" display-message -t "${_rwsess}:${_rwid}" \
    ...
    J)
      if [[ "$_cur_type" == "S" ]]; then
        ...
        CURSOR_ITEM="$_cur_item"
        move_session_down $_sf_idx
    ...
    $'\n' | $'\r')
      if [[ "$_cur_type" == "S" ]]; then
        ...
        for _it2 in "${ITEMS_FLAT[@]}"; do
          if [[ "${_it2%%|*}" == "W" ]]; then
            local _r2="${_it2#*|}"
            local _s2="${_r2%%|*}" _r3="${_r2#*|}" _ss2="${_r3%%|*}"
            [[ "$_s2" == "$_srv" && "$_ss2" == "$_sess" ]] && {
              _first_win="${_s2}:${_r3#*|}"
              break
            }
    ...
    q | Q)
      if [[ -n "$POPUP_MODE" ]]; then
        ...
        $TMUXBIN kill-pane -t "$PANE_ID" 2>/dev/null
        exit 0
      fi
      ;;
  esac
