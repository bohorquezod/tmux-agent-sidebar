...
@test "detect_icon: shell command (zsh) returns E" {
  result=$(detect_icon 99999 "zsh" "" "" "0")
  [ "$result" = "E" ]
}

@test "detect_icon: shell command (bash) returns E" {
  result=$(detect_icon 99999 "bash" "" "" "0")
  [ "$result" = "E" ]
}

@test "detect_icon: shell command (sh) returns E" {
  result=$(detect_icon 99999 "sh" "" "" "0")
  [ "$result" = "E" ]
}

@test "detect_icon: session file status=busy + live process returns W" {
...
@test "detect_icon: session file status=idle returns I" {
  local live_pid=$$
...
@test "detect_icon: session file status=waiting without dialog content returns I" {
  local live_pid=$$
...
@test "detect_icon: session file status=waiting with [Yes] in content returns P" {
  local live_pid=$$
  make_session_file "$live_pid" "waiting"
...
@test "detect_icon: session file status=idle with [Yes] in content returns P" {
  local live_pid=$$
  make_session_file "$live_pid" "idle"
...
@test "detect_icon: session file status=busy + dead process returns X" {
...
@test "detect_icon: pane_dead=1 with no session file returns E" {
  result=$(detect_icon 99999 "node" "" "" "1")
  [ "$result" = "E" ]
}

@test "detect_icon: pane_dead=1 with session file status=busy returns X" {
  make_session_file "99999" "busy"
  result=$(detect_icon "99999" "node" "" "" "1")
  [ "$result" = "X" ]
}

@test "detect_icon: no session file, title starts with ✳ returns I" {
  result=$(detect_icon 99999 "node" "" "✳ Claude" "0")
  [ "$result" = "I" ]
}

@test "detect_icon: no session file, title starts with Braille U+2800 returns W" {
  # ⠋ is U+280B, hex e2a08b (e2a0 prefix)
  result=$(detect_icon 99999 "node" "" "⠋" "0")
  [ "$result" = "W" ]
}

@test "detect_icon: no session file, content contains ⏺ returns W" {
...
@test "detect_icon: no session file, content contains ❯ returns I" {
...
@test "detect_icon: no session file, empty content returns E" {
  result=$(detect_icon 99999 "node" "" "" "0")
  [ "$result" = "E" ]
}
...
@test "check_loop: fewer than 3 W→I transitions → no loop (returns 1)" {
  ...
  printf 'W\n' > "${STATE_DIR}/${wkey}.dprev"

  run check_loop "$wkey" "I"
  [ "$status" -eq 1 ]
}

@test "check_loop: 3 transitions with spacing <60s each → no loop (spacing filter)" {
...
@test "check_loop: 3 transitions with spacing ≥60s within 10 min → loop (returns 0)" {
...
@test "check_loop: loop resets when unread file is absent and icon is I after L" {
